...
def two_level_clustering(
        num_coarse_clusters: int, num_total_clusters: int, vector_db_path: str, all_vector_transforms: list, clustering_niter: int = 25) -> np.ndarray:
...
def train_ivf_index_with_two_level(index: faiss.IndexPreTransform, num_total_clusters: int, vector_dimension: int, vector_db_path: str) -> faiss.IndexPreTransform:
    """
    ...
    centroids = two_level_clustering(
        num_coarse_clusters, num_total_clusters, vector_db_path, all_vector_transforms)
